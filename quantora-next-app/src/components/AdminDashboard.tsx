...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ currentUser, openAuth }) => {
  ...
  const loadData = () => {
    Promise.all([
      ...
      .then(([papersData, usersData]) => {
        setPapers(papersData);
        setUsers(usersData);
      })
      .catch(err => console.error('Failed to load admin workspace data:', err));
  };
  ...
  const handleDelete = (id: string) => {
    if (window.confirm("Are you sure you want to purge this submission from the secure archive?")) {
      fetch(`/api/research/${id}`, {
        ...
        .catch(err => console.error('Failed to purge paper:', err));
    }
  };
  ...
  const handleSaveEdit = (e: React.FormEvent) => {
    ...
    fetch(`/api/research/${editingPaper.id}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        action: 'metadata',
      ...
      .catch(err => console.error('Failed to save metadata edits:', err));
  };
