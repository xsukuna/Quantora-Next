...
const handleMockAuth = (email: string, _method: 'Google' | 'GitHub' | 'OTP'): DBUser => {
  ...
  if (isDeveloperEmail) {
    ...
    localStorage.setItem('quantora_current_user', JSON.stringify(adminUser));
    return adminUser;
  }

  const name = email.split('@')[0].replace('.', ' ').replace(/\b\w/g, c => c.toUpperCase());
  const normalUser: DBUser = {
    id: `user-${Date.now()}`,
    name,
    email,
    role: 'Contributor',
  ...
  localStorage.setItem('quantora_current_user', JSON.stringify(normalUser));
  return normalUser;
};
